...
namespace lucy {
	...
	inline void RayCastPoint(int startX, int startY, int startZ, int endX, int endY, int endZ, RAYCAST_FUNCTION function) {
	...
	inline void RayCastPoint(const glm::ivec3& start, const glm::ivec3& end, RAYCAST_FUNCTION function) {
		RayCastPoint(start.x, start.y, start.z, end.x, end.y, end.z, function);
	}

	inline void RayCastDirection(const glm::ivec3& origin, const glm::vec3& direction, const float& distance, bool normalize, RAYCAST_FUNCTION function) {
		if (normalize) {
