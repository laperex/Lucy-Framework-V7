...
void lra::IntializeRenderer() {
	for (int i = 0; i < MESH_PARTS_COUNT; i++) {
		if (ROBOTIC_ARM_PARTS[i].mesh_id == LUCY_NULL_UUID) {
			ROBOTIC_ARM_PARTS[i].mesh_id = lre::InsertMesh("i", util::LoadMeshToGPU("/home/laperex/Programming/C++/Lucy-Framework-V7-LRA/assets/" + ROBOTIC_ARM_PARTS[i].name + ".stl", "", -ROBOTIC_ARM_PARTS[i].position));
